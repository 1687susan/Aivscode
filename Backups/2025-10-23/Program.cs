...
// 6. SimulationDataService.cs
//    └ 模擬資料服務，提供天氣查詢、員工查詢等模擬回應功能
// 7. agent-config.json 或 config.json
...
// 2. DataStore.cs
//    └ 資料存取層，供插件查詢客戶、訂單等資料
// 3. SimulationDataService.cs
//    └ 模擬資料服務，包含 EmployeeDataService、WeatherDataService 等
// 4. 設定 JSON 檔案（如 config.json）
...
namespace day1
{
    ...
    // 各代理人系統提示
        public class SystemPromptsConfig
        {
            ...
            public string AIScheduler { get; set; } = SystemPrompts.AIScheduler;
        }
    ...
    public static class AIScheduler
    {
        public static async Task<AISchedulingResult> AnalyzeAndSchedule(string userRequest, AgentConfig config)
        {
            ...
            // 從配置中獲取 AI 調度器系統提示，如果沒有則使用預設值
            var systemPrompt = !string.IsNullOrEmpty(config.SystemPrompts.AIScheduler) 
                ? config.SystemPrompts.AIScheduler 
                : SystemPrompts.AIScheduler;
        ...
        public static class AgentManager
        {
            ...
            private static async Task RunSimulatedAssistantAsync(AgentConfig config)
            {
                ...
                while ((input = Console.ReadLine()) is not null)
                {
                    ...
                    // Simple simulation responses using external data service
                    var response = SimulationDataService.GenerateSimulatedResponse(input);
                    Console.WriteLine($"\n🤖 Assistant: {response}\n");
                    Console.Write("Assistant > ");
                }
            }

            // GenerateSimulatedResponse 方法已移至 SimulationDataService.cs

            // GenerateWeatherResponse 方法已移至 WeatherDataService.cs

            // ParseLocationFromInput 方法已移至 WeatherDataService.cs

            // GenerateDetailedWeatherResponse 方法已移至 WeatherDataService.cs

            // GenerateSimulatedWeatherData 方法已移至 WeatherDataService.cs

            /*
            /// <summary>
            /// 生成熱門城市天氣概況 - 已移至 WeatherDataService
            /// </summary>
...
{WeatherDataService.GenerateGlobalWeatherSourceInfo()}";
            }

            /// <summary>
            /// 產生全球天氣資料來源資訊 - 已移至 WeatherDataService
            /// </summary>
            private static string GenerateGlobalWeatherSourceInfo()
            {
                return @"
📊 全球天氣資料來源：
...
🌦️  Visual Crossing: https://www.visualcrossing.com/
...
�️  NOAA National Weather Service: https://weather.gov/
    ...
    // 主程式入口
        class Program
        {
            static async Task Main(string[] args)
            {
                ...
                while (true)
                {
                    ...
                    var choice = input?.Trim().Replace("\uFEFF", "").Replace("\u200B", "") ?? "";
        ...
        public static class MenuHelper
        {
            ...
            private static async Task<bool> ExecuteIntegratedAIResponse(string userRequest, AIExecutionPlan plan, AgentConfig config)
            {
                ...
                // 使用外部 SystemPrompts 類別的整合系統提示
                var integratedPrompt = SystemPrompts.GetIntegratedAIPrompt(userRequest);
